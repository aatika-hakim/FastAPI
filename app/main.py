...
from fastapi import Body, Depends, FastAPI, File, Path, Query, UploadFile, status, Form
...
@app.get("/items/")
async def read_items(commons: CommonsParams = Depends()):
    return commons.__dict__

@app.get("/users/")
async def read_users(commons: CommonsParams = Depends()):
    return commons.__dict__